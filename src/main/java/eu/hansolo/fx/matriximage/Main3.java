...
public class Main3 extends Application {
    ...
    private DotMatrix      transferMatrix;
    private Image          src;
    ...
    @Override public void init() {
        ...
        transferMatrix = DotMatrixBuilder.create()
                                         .colsAndRows(2 * COLS, 2 * ROWS)
                                         .dotOnColor(Color.RED)
                                         .dotShape(DotShape.SQUARE)
                                         .build();
        ...
        for (int y = 0 ; y < ROWS ; y++) {
            for (int x = 0 ; x < COLS ; x++) {
                transferMatrix.setPixel(x, y, greenMatrix.getColorValueAt(x, y));
                transferMatrix.setPixel(x + COLS, y, redMatrix.getColorValueAt(x, y));
        ...
        timer = new AnimationTimer() {
            @Override public void handle(final long now) {
                if (now > lastTimerCall + 10_000_000) {
                    for (int y = 0 ; y < ROWS ; y++) { matrix.setPixel(0, y, transferMatrix.getColorValueAt(x, y)); }
                    matrix.shiftLeft();
                    x++;
                    if (x == 2 * COLS) { x = 0; }
                    lastTimerCall = now;
